...
std::shared_ptr<Personaje> PersonajeFactory::crearPersonaje(const std::string& tipo){
    if(tipo == "Mago"){
        std::string tipoMago = tipoMagoAleatorio();
        if (tipoMago == "Hechicero") {
            return std::make_shared<Hechicero>("Hechicero", tipo, 100, 20);
        } else if (tipoMago == "Conjurador") {
            return std::make_shared<Conjurador>("Conjurador", tipo, 100, 15);
        } else if (tipoMago == "Brujo") {
            return std::make_shared<Brujo>("Brujo", tipo, 100, 10);
        } else if (tipoMago == "Nigromante") {
            return std::make_shared<Nigromante>("Nigromante", tipo, 100, 50);
        }
    }
    else if(tipo == "Guerrero"){
        std::string tipoGuerrero = tipoGuerreroAleatorio();
        if (tipoGuerrero == "Barbaro") {
            return std::make_shared<Barbaro>("Barbaro", tipo, 100, 15);
        } else if (tipoGuerrero == "Paladin") {
            return std::make_shared<Paladin>("Paladin", tipo, 100, 10);
        } else if (tipoGuerrero == "Caballero") {
            return std::make_shared<Caballero>("Caballero", tipo, 100, 30);
        } else if (tipoGuerrero == "Mercenario") {
            return std::make_shared<Mercenario>("Mercenario", tipo, 100, 20);
        }
    }
    return nullptr;
}

std::shared_ptr<Armas> PersonajeFactory::crearArma(const std::string& tipo){
    if(tipo == "ArmaMagica"){
        std::string tipoArmaMagica = tipoArmaMagicaAleatoria();
        if (tipoArmaMagica == "Baston Magico") {
            return std::make_shared<BastonMagico>("Baston Magico", 20, tipo, 10);
        } else if (tipoArmaMagica == "Libro de Hechizos") {
            return std::make_shared<LibroDeHechizos>("Libro de Hechizos", 30, tipo, 10);
        } else if (tipoArmaMagica == "Pocion") {
            return std::make_shared<Pocion>("Pocion", 15, tipo, 10);
        ...
        if (tipoArmaCombate == "Hacha Simple") {
            ...
            return std::make_shared<HachaDoble>("Hacha Doble", 15, tipo, 10);
        } else if (tipoArmaCombate == "Espada") {
            return std::make_shared<Espada>("Espada", 30 , tipo, 10);
        } else if (tipoArmaCombate == "Lanza") {
            return std::make_shared<Lanza>("Lanza", 10, tipo, 10);
        } else if (tipoArmaCombate == "Garrote") {
            return std::make_shared<Garrote>("Garrote", 15, tipo, 10);
        }
    }
    return nullptr;
}

std::shared_ptr<Personaje> PersonajeFactory::PersonajeArmado(const std::string& tipo, const std::string& armaTipo){
